...
const AbuDhabi = () => {
  ...
  useEffect(() => {
    const fetchAbuDhabiMosques = async () => {
      try {
        const mosqueService = MosqueService.getInstance();
        const allMosques = await mosqueService.getAllMosques();
        const abuDhabiMosques = allMosques.filter(mosque => mosque.city === 'Abu Dhabi');
        setMosques(abuDhabiMosques);
      } catch (error) {
        console.error('Error fetching Abu Dhabi mosques:', error);
      } finally {
        setLoading(false);
      }
    };

    fetchAbuDhabiMosques();
  }, []);
